...
## [2026-07-27] apply (drafted) — Buildsafe Software Engineering Manager
- Permanent, in-house EM role at Buildsafe (height safety/scaffolding/construction tech, 25-year-old national Australian business, Arundel/Gold Coast office). JD is explicit the role is not measured by code output — team health, delivery, and business impact instead. Leads engineering across the proprietary BuildCam platform, internal operational systems, and AI initiatives supporting thousands of active construction sites; first three months framed as an establishment/assessment phase before driving longer-term modernisation.
- Fit: STRONG. People-first EM framing (Go1 squad-of-8 hiring/coaching, intelliHR team-of-5 built from scratch, 2sol team-of-5) and Xero's coach-10+-teams-owning-none pattern are direct precedents for "guide technical decisions without becoming the primary coder." AI-assisted development is built evidence (Nagare + Xero AI for Customers), not a claim. Xero API Program/Supergraph is a strong analogue for "modernise business-critical platforms at scale."
- Gaps: PHP and Node.js named in the required stack — genuine, unclaimed, bridged via language-agnostic engineering leadership (same device as Bay Technologies' .NET/C#/Python gap). React/TypeScript softened via Nagare, not closed (standing guardrail). Azure standing gap (AWS Cloud Practitioner still in progress). Construction/height-safety industry is a new vertical, acknowledged in one line, no forced bridge.
- Resume: Format B (narrative, per the retired Format-A-for-contracts heuristic — Format B is now the default for all applications). Xero and Go1 given full narrative treatment; Suncorp/Virgin/intelliHR/Console-Reapit/Emedia condensed to Earlier.
- Cover letter: opens by quoting the JD's own "not measured by how much code you write" line back at it, pairs Xero's coach-without-owning pattern with Go1's direct team-building, uses the AI-assisted-development-built-not-claimed device, and states the PHP/Node.js and construction-industry gaps plainly in one paragraph. Immediate-availability close.
- Pages touched: none yet — awaiting finalise before positioning/keywords/cover-letter-examples updates.
- **Resume redrafted (2026-07-27, same day):** added a standalone Nagare (nagare.com.au) entry between Xero and Go1 — this JD names React, TypeScript, and "AI-assisted development tools" directly in its stack, and Nagare is the wiki's strongest honest answer (built via Claude Code, proficiency explicitly not claimed, per [[projects]] guardrail). Moved 2sol from full narrative to plain bullets to match the Format B six-year cutline (2sol ended 2017, outside the window) — the original draft over-extended narrative treatment to it; Bay Technologies and Opengear both keep 2sol as bullets-only, so this now matches the established EM-archetype convention. Fixed a formatting typo (double space) in the Xero Problem/Solution bullet.
- **Chronology bug caught by Alan, same day:** the first redraft collapsed Suncorp/Virgin/intelliHR/Console-Reapit into the "Earlier" paragraph while keeping 2sol as a full standalone section — but all four of those roles (2017-2020) are chronologically *more recent* than 2sol (2012-2017), so the timeline jumped backward then forward. Corrected to the Opengear/Bay convention: every role gets its own heading in true reverse-chronological order (Xero, Nagare, Go1, Suncorp, Virgin, intelliHR, Console/Reapit, 2sol), and only Emedia + pre-2006 early career collapse into "Earlier." Suncorp given full narrative treatment (sits at the six-year cutline, and its "built governance that didn't exist before I arrived" framing is a strong match for this JD's establishment-phase language). General lesson: when demoting older roles to "Earlier," check every role in between isn't more recent than something still getting a full section.

## [2026-07-27] apply (drafted) — Flight Centre Travel Group Program Manager
- Corporate PMO Program Manager role at Flight Centre Travel Group (FCTG) — matrixed global org, overseeing multiple concurrent local and global projects/programs.
- Fit: STRONG, one of the closest matches in the wiki. 2sol Software Studios (2012-2017) was FCTG's primary engineering partner; Alan personally owned end-to-end delivery of FCTG's own online hotel and accommodation booking platform for five years ($5M+/month, 10+ products, 20+ APIs) — a literal prior relationship with this exact employer, not a bridged analogue. "Matrixed global role — mandatory" is directly evidenced by Xero's dozens-of-teams API Program and Suncorp's four-team offshore-inclusive coordination. 5+ years requirement exceeded several times over (15+).
- Gaps: none of real severity. Named PPM tools (MS Project/Planner/Smartsheet/Wrike) unclaimed — nice-to-have only, not raised. ITIL and Prince2 are standing soft gaps, bridged by SDLC/Agile/PMP+Masters evidence; not raised in the letter given the strength of the rest of the fit.
- Resume: Format B (narrative, current default). Xero and 2sol given full narrative treatment (2sol backed by the Flight Centre connection); Go1 and Suncorp as plain-bullet recent roles; Virgin/intelliHR/Console-Reapit/Emedia condensed to Earlier.
- Cover letter: opens directly on the 2sol/Flight Centre history rather than a JD-interpretation device (closest relative: HomeRepair's parent-company hook) — the strongest direct-company hook in the wiki to date. No gap paragraph; fit is clean enough not to need one. Immediate-availability close.
- Pages touched: none yet — awaiting finalise before positioning/keywords/cover-letter-examples updates.

## [2026-07-27] ingest — Alan-Soto-2026-07-27-Buildsafe-Software-Engineering-Manager-Resume.md
- Resume-only ingest — Alan confirmed the Buildsafe application did not use a cover letter, so the draft cover letter was not archived to `raw/resumes/` and is not reflected in [[cover-letter-examples]].
- Derived from the wiki, no new biographical facts. Captured the chronology-bug correction (every role gets its own heading in true reverse-chronological order; only Emedia + pre-2006 early career collapse into "Earlier") as a reusable process note, and the Buildsafe-specific gaps (PHP/Node.js, Azure, construction/height-safety industry) and strengths (AI-first, "not measured by code output") for future reuse.
- Pages touched: [[positioning]] (new Buildsafe diff-line under the Engineering Manager / Head of Engineering archetype), [[keywords]] (new gap-watch entries under Languages/frameworks, Cloud/infra, Sector/industry; new "strengths JDs mistake for gaps" entry), [[processed]].

## [2026-07-27] apply (finalised) — Buildsafe Software Engineering Manager
- Alan applied without a cover letter. PDF generated via /render (narrative Format B; `build-narrative.js` timed out launching Chrome, fell back to `generate-pdf.js` directly against `resume-narrative.html` per the documented fallback). Resume markdown archived to `raw/resumes/`; cover letter draft left in the application folder only, not archived or ingested.
- Ingested the archived resume (see ingest entry above).

## [2026-07-27] apply (drafted) — Bolton Clarke Senior Project Manager (Technology)
- Not-for-profit aged care provider (88 Residential Aged Care Homes across QLD/NSW/VIC/SA/WA, incl. Allity, McKenzie, Acacia Living Group). Senior/strategic technology role: lead the Residential Aged Care Technology Strategy and future-state blueprint, multi-year roadmaps, enterprise-wide transformation programs, executive/clinical/operational stakeholder engagement, governance/budget/risk/vendor/benefits realisation.
- Fit: MODERATE-TO-STRONG. Xero API Program/Supergraph (target-state architecture, Unified Burndown as a multi-year cross-domain roadmap) is close to a literal precedent for "future-state technology blueprint" — the strongest available match. Suncorp's "governance didn't exist, I built it" story answers the regulated-enterprise/vendor/risk ask directly.
- Gaps: aged care/healthcare is a wholly new vertical, no bridge attempted (role is technology-strategy-led, not clinical, so treated as low-severity). Financial/budget/business-case development is a standing wiki gap, softened via 2sol's $5M+/month commercial ownership. PMP (2012) is lapsed, not active — stated factually alongside the more recent Masters of Project Management rather than overclaimed; JD's "such as" phrasing around PRINCE2/PMP/AIPM CPPD/MSP read as indicative, not a strict live-credential gate.
- Resume: Format B (narrative, current default). Xero and Suncorp given full narrative (Challenge & Impact) treatment; Go1, intelliHR, Console/Reapit, and 2sol as plain bullets in true reverse-chronological order; Virgin Australia cut for relevance/length; Emedia + pre-2006 early career condensed to Earlier.
- Cover letter: opens on the JD's own "technology strategy/blueprint" framing, leads with the Xero Supergraph target-state-architecture story as the direct precedent, backs it with Suncorp's governance-from-scratch/regulated-vendor story, states the aged-care industry gap plainly in one paragraph (no forced bridge), immediate-availability close.
- Pages touched: none yet — awaiting finalise before positioning/keywords/cover-letter-examples updates.
- Pages touched: [[positioning]], [[keywords]], [[processed]] — see ingest entry for detail.

## [2026-07-27] apply (redraft) — Bolton Clarke Senior Project Manager (Technology)
- The application folder had only `gap-analysis.md` and `job-description.md` — no resume or cover letter draft survived from the entry above, so both were rewritten from scratch against the existing gap analysis (still accurate, reused as-is).
- Resume: Format B (narrative). Xero and Suncorp given full Challenge & Impact treatment (target-state architecture/multi-year roadmap; governance-built-from-scratch/regulated-vendor). Go1, Virgin Australia, intelliHR, Console/Reapit, and 2sol kept as plain-bullet sections in true reverse-chronological order rather than cut, to avoid a visible date gap between intelliHR and Suncorp. Emedia + pre-2006 early career condensed to Earlier.
- Cover letter: JD-interpretation + match-claim opener naming the technology-strategy/blueprint framing, three role bullets (Xero target-state architecture, Suncorp governance-from-scratch/regulated vendor, 2sol/BMA tender for vendor and market-engagement evidence), synthesis sentence, one-paragraph aged-care industry gap (no forced bridge), immediate-availability close.
- Pages touched: none — awaiting finalise.

## [2026-07-27] apply (finalised + submitted, backfilled) — Flight Centre Travel Group, Program Manager
- Alan applied to this role before running /finalise — PDFs existed in the application folder but the
  resume/cover letter were never archived to `raw/resumes/`, never ingested, and no `meta.md` existed.
  Ran /finalise retroactively: archived both drafts to `raw/resumes/` as
  `Alan-Soto-2026-07-27-Flight-Centre-Program-Manager-Resume.md` and `…-Cover-Letter.md`, ingested
  both, wrote `meta.md`, and appended `outputs/tracker.md` (status: Submitted / Awaiting response).
- **Proofreading gap:** because /finalise was skipped, the resume-proofreader gate never ran on the
  cover letter before it was sent. The delivered PDF carries two defects — a missing space ("as Lead
  Technical Program Manager at Xero,I directed") and a garbled sentence ("supported by educational
  foundation as Ms of Project Management, and PMP round out the formal side of that experience", a
  broken merge of "a Master's degree" and "PMP round out the formal side"). Both are already sent and
  cannot be corrected; flagged directly to Alan. General lesson: PDFs existing in an application folder
  is not proof /finalise ran — check for `meta.md` and a `raw/resumes/` archive before treating an
  apply as complete.
- Ingest findings: no new biographical facts — first application where 2sol/FCTG is a direct prior
  relationship with the hiring company itself rather than a bridged analogue; captured as a new
  cheat-sheet entry in [[positioning]] and a new entry in [[cover-letter-examples]] (direct
  former-client opener; user confirmed what worked: the opening hook and the labelled-bullet
  scannability, esp. the Xero bullet's density).
- Pages touched: [[positioning]], [[cover-letter-examples]], [[processed]], [[log]], [[tracker]]

## [2026-07-27] apply (drafted) — Fiserv (generic referral resume)
- Not a JD-driven application: a friend at Fiserv (global payments/fintech company) offered to pass Alan's resume along informally for whatever roles are open — program management, project management, or engineering manager. No job posting exists, so no `job-description.md` or `gap-analysis.md` was created for this folder, only `resume-draft.md`.
- Headline: "Technical Program Manager" — chosen as the broadest umbrella covering PM/PjM/EM framings at once (Alan's actual Xero title, and the archetype [[positioning]] already treats as most versatile). Profile paragraph explicitly states comfort across all three altitudes (directing programs, owning project delivery, leading engineering teams directly) rather than committing to one.
- Leaned on the fintech-adjacent cluster device ([[positioning]]) since Fiserv is a payments company: Suncorp (ASX-listed financial services, regulated data migration) and Console/Reapit (payments-provider integration) both foregrounded for genuine payments/financial-services resonance, alongside Xero's US Bills payments-integration work.
- Resume: Format B (narrative, current default). Xero, Go1, and Suncorp given full narrative (Challenge & Impact) treatment — Suncorp included for the fintech/regulated angle even though no specific JD demanded it. Virgin, intelliHR, Console/Reapit, and 2sol as plain bullets in true reverse-chronological order; Emedia + pre-2006 early career condensed to Earlier.
- No cover letter — informal referral distribution, not a submission to a named hiring manager or posting.
- Pages touched: none — this is a generic/no-JD instance, worth adding to [[positioning]]'s TPM archetype as a diff-line if this resume is finalised or leads anywhere.

## [2026-07-27] apply (render) — Bolton Clarke Senior Project Manager (Technology)
- Proofreader gate run before render: flagged US/AU spelling mismatches ("organizational" → "organisational", "standardized" → "standardised" x2), a dangling possessive ("Bolton Clarke's" with nothing following), two double-space typos, and a semicolon misused to join an independent clause to a dependent phrase. Alan chose to fix all six before rendering. A seventh instance of the same US-spelling issue ("organizational restructuring," Suncorp section) was caught separately afterward and fixed too.
- One content issue was flagged but deliberately left as-is at Alan's explicit choice: the cover letter's "Vendor and commercial ownership" bullet says "At Flight Centre, I managed core platforms..." — Alan was actually employed at 2sol Software Studios, Flight Centre's client-facing engineering partner, not Flight Centre directly. See the caution note added to [[positioning]] and [[cover-letter-examples]] — do not reuse this phrasing in future drafts.
- Also found: while fixing the proofreader's flagged double-spaces, two of them turned out to be a mojibake artifact (a UTF-8 non-breaking space mis-decoded into the two literal characters "Â" + U+00A0) rather than plain ASCII double-spaces — likely introduced by whatever tool Alan used to manually edit the draft. Cleaned to plain single spaces.
- PDFs generated via /render (`build-narrative.js`, company name passed explicitly as "Bolton Clarke" since it's a two-word name).

## [2026-07-27] apply (finalised + submitted) — Bolton Clarke Senior Project Manager (Technology)
- Alan applied and confirmed submission. Archived both drafts to `raw/resumes/` as `Alan-Soto-2026-07-27-Bolton-Clarke-Senior-Project-Manager-Resume.md` and `…-Cover-Letter.md`; ingested both. Wrote `meta.md`; appended `outputs/tracker.md` (status: Submitted / Awaiting response).
- Ingest findings: no new biographical facts from the resume (profile years-of-experience phrasing shifted to "over 20 years" — added as a third defensible variant alongside "10+"/"15+" in [[positioning]]'s years-of-experience note). Cover letter added as a new entry to [[cover-letter-examples]] — named-contact "RE:"/"Dear Troy," opener (first time a salutation was sourced directly from a JD's own listed contact rather than a recruiter), and the Karmo/Bay bolded-label bullet device reused in a formal not-for-profit register for the first time. User confirmed what worked: the named-contact opener, the overall warm tone, and specifically the closing line connecting the work to residents' lives and daily care.
- Pages touched: [[positioning]], [[cover-letter-examples]], [[processed]], [[log]], [[tracker]]
